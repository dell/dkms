...
echo 'Adding per-module mixed test module (some modules excluded, others not)'
run_with_expected_output dkms add test/dkms_per_module_mixed_test-1.0 << EOF
Creating symlink /var/lib/dkms/dkms_per_module_mixed_test/1.0/source -> /usr/src/dkms_per_module_mixed_test-1.0
EOF
check_module_source_tree_created /usr/src/dkms_per_module_mixed_test-1.0
run_status_with_expected_output 'dkms_per_module_mixed_test' << EOF
dkms_per_module_mixed_test/1.0: added
EOF

echo 'Testing per-module mixed test module build (should be excluded due to per-module arch restriction)'
run_with_expected_error 77 dkms build -k "${KERNEL_VER}" -m dkms_per_module_mixed_test -v 1.0 << EOF
Warning: The /var/lib/dkms/dkms_per_module_mixed_test/1.0/${KERNEL_VER}/${KERNEL_ARCH}/dkms.conf
for module dkms_per_module_mixed_test/1.0 includes a BUILD_EXCLUSIVE directive
which does not match this kernel/arch/config.
This indicates that it should not be built.
EOF
run_status_with_expected_output 'dkms_per_module_mixed_test' << EOF
...
echo 'Removing per-module mixed test module'
run_with_expected_output dkms remove --all -m dkms_per_module_mixed_test -v 1.0 << EOF
Deleting module dkms_per_module_mixed_test/1.0 completely from the DKMS tree.
EOF
run_status_with_expected_output 'dkms_per_module_mixed_test' << EOF
EOF
remove_module_source_tree /usr/src/dkms_per_module_mixed_test-1.0
